...
TEMP_JS = "/tmp/clean_split.js"
TEMP_SCPT = "/tmp/clean_split.applescript"
...
def ingest_repo_clean(idx):
    ...
    if not files: return "FILE_NOT_FOUND"
    repo_file = files[0]
...
"""
    ...
    status_pop = run_js(js_populate)
    
    # Crucial: Sleep 0.25s for Angular zone change detection to tick
    time.sleep(0.25)
    ...
    status_click = run_js(js_click)
    time.sleep(1.2)
    return f"prep={status_prep}, pop={status_pop}, click={status_click}"

print("Ingesting REPO-006:", ingest_repo_clean(6))
js_verify = """(() => {
...
print("Sources now:", run_js(js_verify))
