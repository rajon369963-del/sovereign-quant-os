...
cur.execute('SELECT count(*) FROM repos_registry')
repos = cur.fetchone()[0]

cur.execute('SELECT count(*) FROM canonical_files')
files = cur.fetchone()[0]

cur.execute('SELECT count(*) FROM duplicate_files')
dupes = cur.fetchone()[0]

cur.execute('SELECT file_name, count(*) as c FROM duplicate_files GROUP BY file_name ORDER BY c DESC LIMIT 10')
top_dupes = cur.fetchall()
...
report_lines = [
    ...
    f'- **325 Physically Cloned Repositories** existed in two disjoint directories (`indian_quant_vault/` with 120 repos and `cloned_trading_wheels/` with 205 repos).',
    f'- **31,654 Duplicate Files** were scattered across directories with contradictory risk parameters and unsynchronized execution logic.',
    ...
    '',
    f'- **Total Registered Repositories:** {repos}',
    f'- **Total Canonical Unique Files (SHA-256):** {files}',
    f'- **Total Duplicate Files Flagged & Isolated:** {dupes}',
...
for name, c in top_dupes:
    report_lines.append(f'- `{name}`: {c} duplicate instances')
...
report_path = '/Users/rajondas/teamwork_projects/sovereign-quant-os/TRADING_MASTER_CONSOLIDATION_AUDIT.md'
with open(report_path, 'w') as f:
    f.write('\n'.join(report_lines) + '\n')

print(f'Wrote clean audit report to {report_path}')
conn.close()
