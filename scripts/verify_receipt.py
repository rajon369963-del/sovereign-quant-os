...
def verify_live_benchmark(bench_path: Path) -> bool:
    ...
    checks_min = 1_000_000.0 if is_arm else 100_000.0
    live_checks = float(data.get("throughput_checks_sec", 0))

    if live_checks < checks_min:
        print(f"❌ FAIL: Live Risk Gate throughput too low: {live_checks:,.1f} checks/s < min {checks_min:,.1f}")
        return False

    print(f"• Live Benchmark Binding  : CAUSAL LINK ESTABLISHED (Live Risk Gate={live_checks:,.1f} checks/s, Avg Latency={data.get('avg_latency_us')} µs) [PASS]")
    return True
...
def extract_visible_svg_text(svg_raw: str) -> str:
    ...
    # 2. Extract strictly from <text> and <tspan> tags
    extracted_tokens = []
    try:
        ...
        # Resilient fallback regex matching text node inner content
...
def verify(repo_root: Path, live_bench_file: Path = None) -> bool:
    ...
    if live_bench_file:
        ...
        default_bench = repo_root / "scripts" / "quant_benchmark_results.json"
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Quant OS Truth Guard")
