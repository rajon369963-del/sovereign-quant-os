...
class TradeOrder:
    ...
    def apply_fill_fragment(self, fragment_id: str, fragment_qty: float, fragment_price: float) -> bool:
        ...
        self.fill_events.append({"fragment_id": fragment_id, "quantity": fragment_qty, "price": fragment_price, "timestamp": time.time()})
...
class ExecutionDaemon:
    def __init__(self, db_path: str = "/Users/rajondas/.gemini/antigravity/scratch/antigravity_yolo_trading_engine/trading_ledger.sqlite", max_account_drawdown: float = 10000.0, daily_loss_limit_pct: float = 0.02, initial_capital: float = 10000.0):
    ...
    def _trigger_kill_switch(self, reason: str):
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("INSERT INTO circuit_breaker_events (timestamp, trigger_reason, equity, drawdown) VALUES (?, ?, ?, ?)", (time.time(), reason, self.current_equity, self.peak_equity - self.current_equity))
    ...
    def dispatch_order_with_self_healing(self, signal: TradeSignal, gate_res: RiskGateResult) -> TradeOrder | None:
        ...
        order = TradeOrder(order_id=order_id, symbol="NIFTY_FUT", strategy=signal.strategy.value, side=signal.signal_type.value, quantity=gate_res.approved_quantity, entry_price=signal.price, hard_stop_loss=gate_res.hard_stop_loss, hard_take_profit=gate_res.hard_take_profit, state=OrderState.PENDING, created_at=time.time(), filled_quantity=0.0, remaining_quantity=gate_res.approved_quantity)
    ...
    def dispatch_with_reconciliation(self, client_order_id: str, fake_venue: Any, signal: TradeSignal, quantity: float) -> dict[str, Any]:
        ...
        if attempt_1_res.get("error") == "TIMEOUT_OR_LOST_RESPONSE":
            ...
            if reconcile_state.get("status") in ("ACCEPTED", "FILLED"):
                return {"status": reconcile_state["status"], "client_order_id": client_order_id, "reconciled": True, "resubmission_suppressed": True}
            elif reconcile_state.get("status") == "UNKNOWN":
                return {"status": "HOLD/RECONCILE_REQUIRED", "client_order_id": client_order_id, "reconciled": True, "resubmission_suppressed": True}
                ...
                return {"status": attempt_2_res.get("status", "REJECTED"), "client_order_id": client_order_id, "reconciled": True, "attempt": 2}
    ...
    def simulate_price_tick(self, current_price: float, risk_gate: RiskGatekeeper):
        ...
        for order in orders_to_close:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                """, (order.order_id, order.symbol, order.strategy, order.side, order.quantity, order.entry_price, order.fill_price, order.exit_price, order.hard_stop_loss, order.hard_take_profit, order.realized_pnl, order.state.value, order.created_at))
