#!/usr/bin/env python3
"""
⚡ UNIFIED THURSDAY EXPIRY PIPELINE ORCHESTRATOR
=================================================
Physical Master Runner for Thursday 0-DTE Expiry Trading on Indian NSE Equities (DhanHQ API v2).
Ties together the complete causal chain:
  [08:35 AM] Pre-Market Strategy Compiler (Dual-Hypergraph RAG + AST + Backtest + 10x Stress)
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] [THURSDAY_EXPIRY] %(message)s",
    handlers=[
        logging.StreamHandler(sys.stdout),
        logging.FileHandler(PROJECT_DIR / "thursday_expiry_pipeline.log", encoding="utf-8"),
    ],
)
logger = logging.getLogger("ThursdayExpiryOrchestrator")
...
class ThursdayExpiryOrchestrator:
    def __init__(self, dry_run: bool = True, capital: float = None):
        self.dry_run = dry_run
        self.db_path = DB_PATH
        self.capital = capital if capital is not None else self._fetch_live_capital()
        self._init_ledger()

    def _fetch_live_capital(self) -> float:
        try:
            bridge = DhanLiveBridge()
            if bridge.is_connected:
                bal_resp = bridge.check_balance()
                if bal_resp.get("status") == "SUCCESS":
                    data = bal_resp.get("data", {}).get("data", {})
                    avail = float(data.get("availabelBalance", data.get("availableBalance", 918.43)))
                    logger.info(f"✓ Retrieved Live Dhan Capital: ₹{avail:.2f}")
                    return avail
        except Exception as e:
            logger.warning(f"Could not query live Dhan funds ({e}), defaulting to ₹918.43")
        return 918.43
    ...
    def _init_ledger(self):
        ...
        cur.execute("""
        CREATE TABLE IF NOT EXISTS thursday_expiry_pipeline_ledger (
    ...
    def stage_1_premarket_compilation(self) -> dict[str, Any]:
        ...
        synthesizes SovereignAdaptiveAlpha, validates AST, runs backtest & 10x stress test.
        """
        logger.info("=" * 70)
    ...
    def stage_2_hard_freeze_gate(self) -> dict[str, Any]:
        ...
        # Invariant Verification: All symbols must be BIDIRECTIONAL for Thursday 0-DTE Expiry
        for sym, cfg in matrix.items():
            cfg["sector_bias"] = "BIDIRECTIONAL"
            if cfg.get("chandelier_multiplier", 1.2) > 1.5:
                cfg["chandelier_multiplier"] = 1.2
            cfg["max_risk_pct"] = 0.025  # 2.5% Half-Kelly
        ...
        logger.info(f"✓ Stage 2 Passed: Strategy locked with SHA-256: {strategy_hash[:16]}... ({len(matrix)} symbols frozen)")
        return freeze_receipt

    def stage_3_pre_open_amnesia_protocol(self, simulated_gap_pct: float = -0.42) -> dict[str, Any]:
        """
    ...
    def stage_4_opening_wick_quarantine(self, duration_seconds: int = 65, simulate: bool = True) -> dict[str, Any]:
        ...
        logger.info("✓ Stage 4 Passed: 65-second opening quarantine verified. Market noise dissipated.")
        return {
    ...
    def stage_5_live_armed_execution(self) -> dict[str, Any]:
        ...
        2. 3-Gate Variance Shield:
           - Gate 1: Half-Kelly 2.5% max risk per trade (<= ₹23.83 on ₹953.11)
           - Gate 2: ₹50 max daily loss cap (preserving ₹953.11 capital)
        ...
        max_trade_risk = round(self.capital * 0.025, 2)
        logger.info("✓ 3-Gate Variance Shield Specifications Active:")
        logger.info(f"  • Sovereign Preserved Capital: ₹{self.capital:.2f}")
        logger.info(f"  • Gate 1 (Single Trade Risk):  Max ₹{max_trade_risk:.2f} (2.5% Half-Kelly)")
    ...
    def run_full_expiry_orchestration(self, simulate_gap: float = -0.42) -> dict[str, Any]:
        start_time = datetime.now(IST)
        run_id = f"EXPIRY_PIPE_THU_{start_time.strftime('%Y%m%d_%H%M%S')}"
        logger.info(f"Starting Unified Thursday Expiry Pipeline: {run_id}")
        ...
        cur = conn.cursor()
        cur.execute("""
        INSERT INTO thursday_expiry_pipeline_ledger
        ...
        logger.info("=" * 70)
        logger.info(f"🏆 THURSDAY PIPELINE EXECUTION COMPLETE: {run_id}")
...
def main():
    parser = argparse.ArgumentParser(description="Unified Thursday Expiry Pipeline Orchestrator")
    parser.add_argument("--live", action="store_true", help="Run with live DMA broker dispatch")
    parser.add_argument("--simulate-gap", type=float, default=-0.42, help="Simulate pre-open gap percentage (default: -0.42%)")
    parser.add_argument("--capital", type=float, default=None, help="Sovereign preserved capital (default: auto-read from Dhan)")
    args = parser.parse_args()

    orchestrator = ThursdayExpiryOrchestrator(dry_run=not args.live, capital=args.capital)
